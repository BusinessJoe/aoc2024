...
pub fn aoc8(allocator: std.mem.Allocator, reader: anytype) !Answers {
    const grid = try input.Grid.fromReader(allocator, reader);
    defer grid.deinit();
...
pub fn main() !void {
    ...
    const answers = try aoc8(allocator, stdin.reader());
...
test "test example" {
    ...
    const answers = try aoc8(test_allocator, stream.reader());
